    ...
    for j= 1:center:(n-cell-1)
       ...
       center_matrix=map((i+center_x):(i+center_x+center-1),(j+center_x):(j+center_x+center-1));
           if sum(sum(center_matrix))<25;
            fullFileName = fullfile('~/Desktop/xianpostrain/', outputFileName); 
            imwrite(img3, ['xianpostrain/',outputFileName]); 
           else
            fullFileName = fullfile('xiannegtrain/', outputFileName);
           imwrite(img3, ['~/Desktop/xiannegtrain/',outputFileName]);
           end
